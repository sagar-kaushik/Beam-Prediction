...
% locations should be a 2D array with the following dimensions: 3 X # of users. 
...
for ant = 1:length(num_ant_mm)% number of antennas to loop over
	...
	options.dataFile1 ='DeepMIMO_dataset_3p5.mat';% The path to the sub-6 data file
    options.dataFile2 ='DeepMIMO_dataset_28.mat';% The path to the mmWave data file
	...
	for p = 1:length(tx_power)
		...
		fileName = struct('name',{options.dataFile1,...
		                          options.dataFile2});
		[dataset,options] = dataPrep(fileName, options);
        filename=['Dataset_' num2str(tx_power(p)) 'dB.mat'];
        save(filename,'dataset','options', '-v7.3');
        ...
        variable_name = [options.expTag '_results_' num2str(snr_db(p)) '.mat'];
...
% % Plotting accuracies
% variable_name = [options.expTag '_results_' num2str(length(tx_power))];
% load(variable_name)
